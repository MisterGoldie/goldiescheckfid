...
async function resolveAddress(input: string): Promise<string> {
  ...
  if ((input as string).endsWith('.eth')) {
    const provider = new ethers.JsonRpcProvider(ALCHEMY_MAINNET_URL);
    try {
      const address: string | null = await provider.resolveName(input);
      if (address) {
        return address;
      }
    } catch (error) {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', width: '100%', height: '100%', backgroundColor: '#FF8B19', padding: '20px', boxSizing: 'border-box' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', marginBottom: '20px' }}>
          {pfpUrl ? (
            ...
            <div style={{ width: '64px', height: '64px', borderRadius: '50%', marginRight: '10px', backgroundColor: '#ccc', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px', fontWeight: 'bold', color: '#fff' }}>
              {displayName ? displayName[0] : '?'}
            </div>
          )}
          <h2 style={{ fontSize: '32px', textAlign: 'center' }}>{displayName || 'User'}</h2>
        </div>
        <p style={{ fontSize: '36px', marginBottom: '20px', textAlign: 'center' }}>{balanceDisplay}</p>
        <p style={{ fontSize: '28px', textAlign: 'center' }}>{usdValueDisplay}</p>
      </div>
    ),
    intents: [
